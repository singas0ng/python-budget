...
class Category(SQLModel, table=True):
    ...
    reg_date: datetime              = Field(sa_column=Column(comment="생성일자", type_=DATETIME), default_factory=lambda: datetime.now()) #생성될 때 Field가 null 이면 default 값으로 넣어달라
    modify_date: Optional[datetime] = Field(sa_column=Column(comment="수정일자", type_=DATETIME), default_factory=datetime.now)